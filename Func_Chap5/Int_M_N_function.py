...
def Int_M_N_I_y_y_cl1_cl2(A = None, A_w = None, b = None, t_f = None, t_w = None, h_w = None, welded_rolled = None,  
                    fy = None, N_Ed = None):
    ...
    while True:
        if N_Ed is None:
            N_Ed = float(input("N_Ed (tbd: 0): "))
        if N_Ed == 0 and M_N_Rd is None:
            print("Determining N_Ed")
            M_N_Rd = float(input("M_N_y,Ed: "))
        
        if N_Ed ==0 and M_N_Rd ==0:
            print("M_N,y,Rd and N_Ed are 0")
            input("Any key to continue...")
            continue
        else:
            break

    A_w= A_w_I_H(A= A, A_w= A_w, b= b, t_f= t_f, t_w= t_w, h_w= h_w, welded_rolled= welded_rolled)
...
def Int_M_N_Rect_Tube_y_y_cl1_cl2(A = None, A_w = None, b = None, t_f = None, fy = None, N_Ed = None):
    
    print("Int M-N Calc, Rect/Tube y-y")
    if A is None:
        A = float(input("A: "))
    if A_w is None:
        A_w = float(input("A_w: "))
    ...
    print("M_N,y,Rd= ", M_N_Rd)
    return M_N_Rd

def Int_M_N_Rect_Tube_z_z_cl1_cl2(A = None, A_w = None, b = None, t_w = None, h = None, fy = None, N_Ed = None):
    
    print("Int M-N Calc, Rect/Tube z-z")
    if A is None:
        A = float(input("A: "))
    if A_w is None:
        A_w = float(input("A_w: "))
